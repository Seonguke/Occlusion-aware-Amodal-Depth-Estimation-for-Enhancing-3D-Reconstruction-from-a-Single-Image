...
def write_detection_image(image: Union[np.array, torch.Tensor], detections: BoxList, output_file: os.PathLike) -> None:
    ...
    masks =[mask.get_mask_tensor().numpy() for mask in masks]
    #
    # # TODO: clean up
    ...
    # masks = detections["masks"][0].detach().cpu().numpy()
    _, H, W = image.shape

    ##masks = SegmentationMask(detections["masks"][0], [W, H])

    # dishow(image)
    # zzz=np.stack([mask.get_mask_tensor().numpy() for mask in xxx])
    # for mask in masks:
    #     #print(mask)
    #     dishow(mask)
    # TODO: clean up
    ...
    image = overlay_detections(image, bounding_box, masks, label)
    #dishow(image)
    #io.write_image(image, output_file)
...
def overlay_detections(image: np.array, boxes: np.array, masks: np.array, class_ids: np.array) -> np.array:
    ...
    for i in range(num_instances):
        ...
        if masks is not None:
            ...
            padded_mask = np.zeros((masked_image.shape[0] + 2, masked_image.shape[1] + 2), dtype=np.uint8)
    ...
    image_from_plot = masked_image.astype(np.uint8)
