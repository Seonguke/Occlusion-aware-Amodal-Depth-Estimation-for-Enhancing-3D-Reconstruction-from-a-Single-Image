...
class DepthMap(object):
    ...
    def to_pointcloud(self, filename,camera2frustum=None):

        pointcloud, _ = self.compute_pointcloud(camera2frustum)
    ...
    def compute_pointcloud(self,camera2frustum=None):
        ...
        if camera2frustum is not None:
            camera2frustum=camera2frustum.cuda()
            pointcloud = torch.mm(torch.inverse(self.intrinsic_matrix.float()), coords3d.float())

            #print()
            pointcloud = torch.mm(camera2frustum, pointcloud.float().cuda()).t()[:, :3]
